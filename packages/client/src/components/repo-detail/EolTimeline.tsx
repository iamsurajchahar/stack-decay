...
function getEolBarColor(days: number | null, isEol: boolean): string {
  if (isEol || (days !== null && days < 0)) return 'bg-gray-900 dark:bg-gray-300';
...
function getStatusIcon(days: number | null, isEol: boolean) {
  if (isEol || (days !== null && days < 0)) return <XCircle className="h-4 w-4 text-gray-900 dark:text-gray-300" />;
...
export function EolTimeline({ repoId }: EolTimelineProps) {
  ...
  if (isLoading) {
    return (
      <div className="card">
        <h3 className="mb-4 text-base font-semibold text-gray-900 dark:text-white">EOL Timeline</h3>
          ...
          return (
            <div key={entry.product} className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <div className="relative h-6 w-full overflow-hidden rounded-full bg-gray-100 dark:bg-gray-700">
                  <div
                    className={clsx(
              ...
              <div className="w-36 flex-shrink-0 text-sm text-gray-500 dark:text-gray-400">
                {entry.eolDate && (
                ...
                <div
                  className={clsx(
                    'text-xs font-medium',
                    entry.isEol && 'text-gray-900 dark:text-gray-200',
                    !entry.isEol && entry.daysUntilEol !== null && entry.daysUntilEol < 90 && 'text-red-600 dark:text-red-400',
                    !entry.isEol && entry.daysUntilEol !== null && entry.daysUntilEol >= 90 && entry.daysUntilEol < 180 && 'text-orange-600 dark:text-orange-400',
                    !entry.isEol && entry.daysUntilEol !== null && entry.daysUntilEol >= 180 && entry.daysUntilEol < 365 && 'text-yellow-600 dark:text-yellow-400',
                    !entry.isEol && (entry.daysUntilEol === null || entry.daysUntilEol >= 365) && 'text-green-600 dark:text-green-400',
                  )}
                >
